  ...
  fetch("/admin/list")
    ...
    .then((data2) => {
      ...
      for (let i = 0; i < data2.length; i++) {
        ...
        count_all++;
        if (member.waitlist == 1) count_competing++;
        else if (member.waitlist > 1) count_waitlist++;
      ...
      document.querySelector(".right").innerHTML +=
        `<br>${count_competing} competing, ${count_waitlist} waitlisted, ${count_all} total`;
    });
}
...
function showUser(userId) {
  ...
  // Statuses
  document.getElementById("statusColor").style.color =
    user.waitlist === 1 ? "#51cd7f" : "#cf565f";
  ...
  // Demography
  document.getElementById("comp_day").innerText = user.assigned_run
    ? user.assigned_run
    : "Unassigned: Prefers " + user.availability.replace(" works.", "");
  document.getElementById("team_number").innerText = user.team_number
    ? user.team_number
    : "Unassigned";
  document.getElementById("team_name").innerText = user.team_name
    ? user.team_name
    : "Solo";
  ...
  document.getElementById("acceptanceLabel").innerText =
    user.waitlist !== 1 ? "Force Accept" : "Force Drop Out";
  ...
  document.getElementById("checkinLabel").innerText = user.checked_in
    ? "Check Out"
    : "Check In";
...
function mentorFilter(isMentorMode) {
  ...
  userList.filter((item) => {
    let activeOrInactive = !(
      item.values().teamnumber && item.values().assignedrun
    );
    if (!isMentorMode) {
  ...
  document.getElementById("menteeFilter").innerText = isMentorMode
    ? "Unassigned"
    : "Any Assign";
